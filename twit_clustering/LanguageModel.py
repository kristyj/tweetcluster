...
class LanguageModel(object):
    ...
    Can print to ARPA files"""
    startsymbol="<s>"; endsymbol="</s>"

    def __init__(self, documents, n, lm, smoothing, holdbuild=False):
        """Build the language model based on the document else make a blank model"""
        print(documents)
        if type(documents) != list: #document is filename
            ...
            self.lm_params = LMParams(n, lm, smoothing) #now lm_params holds all the info about things that need to be trained
    ...
    # def count_to_populate(self):
    #     self.ngrams = LanguageModel.build_ngram_counts(self.input, self.order,
    #                                                    buildlowerorder=self.lm_params.save_lower_orders,
    #                                                    givelines=)

    @staticmethod
    def build_ngram_counts(textinput_iterator, n,  buildlowerorder=True, givelines=False, givedf=False):
        #TODO: May contain extra start/end tokens
        '''Put together dictionaries of ngrams and how often they occur, where n is the order.
        This iterates n once over the text and makes n dictionaries if buildlowerorder is True.'''
        print("building ngram counts")
        totalwords = 0
        lines = 0; df = {}
        ...
        myCounters = {m :  Counter() for m in range_to_build} #build descending ngram dicts
    ...
    @staticmethod
    def calc_vocab_size(ngram_dict):
        return {i: len(ngram_dict[i].keys()) for i in ngram_dict.keys()}
...
class GlobalLanguageModel(LanguageModel):
    ...
    def increment_counts(self, sentence_list):
        n = self.order
        range_to_build = (range(n, 0, -1) if self.lm_params.buildlowerorder else range(n, n-2, -1))
        ...
        #TODO: update count_of_counts, wc, vocabsize
...
if __name__=="__main__":
    #testing
    mylm = GlobalLanguageModel([['This','is','an','example','sentence'],["this","is","an","example", "blackbird"]], 3, 'good-turing', 'none', )
    print(mylm.ngrams)
    print('count_of_counts',mylm.count_of_counts)
    print(".docfreq", mylm.docfreq)
    print(".vocabsize",mylm.vocabsize)
    a = mylm.give_gram_prob(["an", "example", "sentence"])
    b = mylm.give_gram_prob(["is", "an", "poo"])
